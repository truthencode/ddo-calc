...
case class PlayerAttribute(override val item: Attribute, override val baseValue: Int = 0)
    extends AttributeStat {
  override def value: Int = ???
}

case class PlayerStrAttribute(override val baseValue: Int = 0) extends AttributeStat {
...
object ThrowAway {
  ...
  val m: Map[T, List[MyPair]] = Map(
    Attribute.Strength -> List((EffectParameter.BonusType(BonusType.ActionBoost), 5))
  )
